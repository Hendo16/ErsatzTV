...
public class SearchRepository : ISearchRepository
{
    ...
    public IAsyncEnumerable<MediaItem> GetAllMediaItems()
    {
        ...
        return dbContext.MediaItems
            ...
            .ThenInclude(ovm => ovm.Streams)
            .Include(mi => (mi as FillerMediaItem).FillerMetadata)
            .ThenInclude(fm => fm.Genres)
            .Include(mi => (mi as FillerMediaItem).FillerMetadata)
            .ThenInclude(fm => fm.Tags)
            .Include(mi => (mi as FillerMediaItem).FillerMetadata)
            .ThenInclude(fm => fm.Studios)
            .Include(mi => (mi as FillerMediaItem).FillerMetadata)
            .ThenInclude(fm => fm.Actors)
            .Include(mi => (mi as FillerMediaItem).FillerMetadata)
            .ThenInclude(fm => fm.Directors)
            .Include(mi => (mi as FillerMediaItem).FillerMetadata)
            .ThenInclude(fm => fm.Writers)
            .Include(mi => (mi as FillerMediaItem).FillerMetadata)
            .ThenInclude(fm => fm.Guids)
            .Include(mi => (mi as FillerMediaItem).MediaVersions)
            .ThenInclude(fm => fm.Streams)
            .Include(mi => (mi as Song).SongMetadata)
            .ThenInclude(mm => mm.Tags)
            .Include(mi => (mi as Song).SongMetadata)
            .ThenInclude(mm => mm.Genres)
