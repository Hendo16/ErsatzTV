...
public class SearchRepository(IDbContextFactory<TvContext> dbContextFactory) : ISearchRepository
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var item in GetAllFillers(cancellationToken))
        {
            yield return item;
        }
    ...
    private async IAsyncEnumerable<FillerMediaItem> GetAllFillers(
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        await using var dbContext = await dbContextFactory.CreateDbContextAsync(cancellationToken);

        ConfiguredCancelableAsyncEnumerable<FillerMediaItem> fillers = dbContext.FillerMediaItems
            .AsNoTracking()
            .TagWithCallSite()
            .IncludeForSearch()
            .AsSplitQuery()
        ...
        await foreach (var filler in fillers)
        {
            yield return filler;
        }
    }
